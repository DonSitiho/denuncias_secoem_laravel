        ...
        <div class="card shadow-sm">
            <div class="card-header border-0 pt-6">
                ...
                <div class="card-toolbar">
                    ...
                    @livewire('uaoic-denuncias.acciones-botones', ['estatus' => $denuncia->id_estado, 'id_denuncia' => $denuncia->id_denuncia], key('acciones-'.$denuncia->id_denuncia))

                    @if ($denuncia->id_estado == 2)
                        {{-- 1. Botón/Modal de Turno (Permiso: uaoic-denuncia-turnar) --}}
                    ...
                    @elseif ($denuncia->id_estado == 3)
                        ...
                        @can('uaoic-denuncia-descargar')
                            <a href="{{ route('uaoic.exportar.expediente', $denuncia->id_denuncia) }}"
                                class="btn btn-sm btn-light-warning me-2">
                                <i class="fas fa-file-pdf me-1"></i> Exportar Expediente
                            </a>
                        @endcan

                        {{-- 2. Botón/Modal de Solventar informacion (Permiso: uaoic-denuncia-solventar-info) --}}
                        @can('uaoic-denuncia-solventar-info')
                            {{-- El modal debe estar incluido como partial --}}
                            @include('uaoic-denuncias.partials.modal_solicitar_info', [
                                'denuncia' => $denuncia,
                            ])
            ...
            <div class="card-body pt-0">
                ...
                <ul class="nav nav-tabs nav-line-tabs mb-5 fs-6">
                    ...
                    <li class="nav-item"><a class="nav-link" data-bs-toggle="tab" href="#tab_evidencia"><i
                                class="fas fa-paperclip me-2"></i> Evidencia ({{ $denuncia->archivos->count() }})</a>
                    </li>
                    ...
                    <li class="nav-item"><a class="nav-link" data-bs-toggle="tab" href="#tab_solventar_info"><i
                                class="fas fa-id-card me-2"></i> Solventar Info</a></li>
                </ul>
                ...
                <div class="tab-content" id="denunciaTabsContent">
                                ...
                                <div class="card card-dashed shadow-sm">
                                    ...
                                    <div class="card-body">
                                        <div class="fs-6 py-2">
                                            ...
                                            <span
                                                class="ms-2">{{ $denuncia->circunstancia->fecha_hechos->format('d/m/Y') ?? 'N/A' }}
                                                <small
                                                    class="text-muted">({{ $denuncia->circunstancia->hora_hechos ?? 'Sin hora' }})</small>
                                            </span>
                                        </div>
                                        <div class="fs-6 py-2">
                                            <strong>{{ __('Lugar (Municipio):') }}</strong>
                                            <span
                                                class="ms-2">{{ $denuncia->circunstancia->municipio->nombre_municipio ?? 'No Especificado' }}</span>
                                        </div>
                                        <div class="fs-6 py-2">
                                            <strong>{{ __('Localidad / Colonia:') }}</strong>
                                            <span
                                                class="ms-2">{{ $denuncia->circunstancia->localidad ?? 'N/A' }}</span>
                                        </div>
                                        <div class="fs-6 py-2">
                                            <strong>{{ __('Dirección Exacta:') }}</strong>
                                            <span
                                                class="ms-2">{{ $denuncia->circunstancia->direccion_exacta ?? 'N/A' }}</span>
                                        </div>
                                        <div class="fs-6 py-2">
                                            <strong>{{ __('Dependencia Señalada:') }}</strong>
                                            <span
                                                class="ms-2">{{ $denuncia->circunstancia->dependencia_involucrada ?? 'N/A' }}</span>
                                        </div>
                                        <div class="fs-6 py-2">
                                            <strong>{{ __('Trámite / Solicitud:') }}</strong>
                                            <span
                                                class="ms-2">{{ $denuncia->circunstancia->tramite_solicitado ?? 'N/A' }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="card card-dashed shadow-sm">
                                    ...
                                    <div class="card-body">
                                        <p><strong>{{ __('Circunstancias Detalladas:') }}</strong>
                                            {{ $denuncia->circunstancia->circunstancias_detalladas ?? 'Sin detalle adicional' }}
                                        </p>
                                        <p><strong>{{ __('Programa Público:') }}</strong>
                                            {{ $denuncia->programa_publico ?? 'No relacionado a programa' }}</p>
                                        <p><strong>{{ __('Daño Económico:') }}</strong> <span
                                                class="badge badge-light-danger fs-5">${{ number_format($denuncia->dinero_solicitado, 2) }}</span>
                                        </p>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="tab-pane fade" id="tab_involucrados" role="tabpanel">
                        ...
                        @forelse ($denuncia->involucrados as $involucrado)
                            <div class="card card-dashed mb-5 bg-light-primary">
                                ...
                                <div class="card-body py-3">
                                    <div class="row fs-6">
                                        <div class="col-md-4"><strong>Puesto:</strong>
                                            {{ $involucrado->puesto_denunciado ?? 'N/A' }}</div>
                                        <div class="col-md-4"><strong>Sexo:</strong> {{ $involucrado->sexo ?? 'N/A' }}
                                        </div>
                                        <div class="col-md-4"><strong>Edad Aprox:</strong>
                                            {{ $involucrado->edad_aprox ?? 'N/A' }}</div>
                                        <div class="col-md-12 mt-3"><strong>Descripción Física:</strong>
                                            {{ $involucrado->descripcion_fisica ?? 'Sin descripción adicional.' }}
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        @forelse ($denuncia->testigos as $testigo)
                            <div class="card card-dashed mb-5 bg-light-success">
                                <div class="card-header">
                                    <h5 class="card-title">
                                        {{ $testigo->nombre_testigo ?? 'Testigo Anónimo/Sin Nombre' }}</h5>
                                </div>
                                <div class="card-body py-3">
                                    <p><strong>Contacto:</strong> {{ $testigo->datos_contacto ?? 'N/A' }}</p>
                                    <p><strong>Observaciones:</strong>
                                        {{ $testigo->observaciones ?? 'Sin observaciones.' }}</p>
                                </div>
                            </div>
                        @empty
                            <div class="alert alert-light">
                                {{ __('El denunciante no proporcionó información sobre testigos.') }}</div>
                        @endforelse
                    ...
                    <div class="tab-pane fade" id="tab_evidencia" role="tabpanel">
                        @forelse ($denuncia->archivos as $archivo)
                            <div class="d-flex align-items-center bg-light-info rounded p-5 mb-3">
                                <span class="svg-icon svg-icon-2hx svg-icon-info me-5">
                                    <i
                                        class="fas fa-file-{{ $archivo->tipo_archivo == 'imagen' ? 'image' : ($archivo->tipo_archivo == 'documento' ? 'alt' : 'video') }} fs-1 text-info"></i>
                                </span>
                                <div class="flex-grow-1">
                                    <a href="#"
                                        class="fw-bold text-gray-800 text-hover-primary fs-6">{{ $archivo->nombre_original }}</a>
                                    <span class="text-muted fw-semibold d-block">Tipo:
                                        {{ strtoupper($archivo->tipo_archivo) }} | Carga:
                                        {{ $archivo->fecha_carga->format('d/m/Y') }}</span>
                                </div>
                                <span class="ms-2">
                                    ...
                                    @can('uaoic-denuncia-descarga')
                                        <a href="{{ route('uaoic.descargar.evidencia', ['id_archivo' => $archivo->id_archivo]) }}"
                                            class="btn btn-icon btn-sm btn-info" title="Descargar Evidencia">
                        ...
                        @empty
                            <div class="alert alert-warning">
                                {{ __('No se adjuntaron archivos de evidencia en esta denuncia.') }}</div>
                        @endforelse
                    </div>
                    ...
                    <div class="tab-pane fade" id="tab_contacto_info" role="tabpanel">
                        <div class="card card-dashed shadow-sm">
                            <div class="card-header">
                                <h4 class="card-title">{{ __('Datos de Contacto del Ciudadano') }}</h4>
                            </div>
                            <div class="card-body">
                                ...
                                @elseif ($denuncia->contacto)
                                    <div class="fs-5 py-2"><strong>{{ __('Nombre Completo:') }}</strong>
                                        {{ $denuncia->contacto->nombre_completo }}</div>
                                    <div class="fs-5 py-2"><strong>{{ __('Correo Electrónico:') }}</strong>
                                        {{ $denuncia->contacto->correo_electronico }}</div>
                                    <div class="fs-5 py-2"><strong>{{ __('Teléfono:') }}</strong>
                                        {{ $denuncia->contacto->telefono }}</div>
                                @else
                                    <div class="alert alert-warning">
                                        {{ __('Denuncia identificada, pero los datos de contacto no fueron guardados o son nulos.') }}
                                    </div>
                                @endif
                            </div>
                        </div>
                    ...
                    {{-- ========================================================================= --}}
                    {{-- PESTAÑA 5: INFORMACION SOLICITADA AL DENUNCIANTE --}}
                    {{-- ========================================================================= --}}
                                ...
                                <div class="card card-dashed shadow-sm">
                                    ...
                                    <div class="card-body">
                                        <div class="fs-6 py-2">
                                            ...
                                            <span class="ms-2">{{ $info->fecha_solicitud_info->format('d/m/Y') ?? 'N/A' }} 
                                                <small class="text-muted">({{ $info->fecha_solicitud_info->format('H:i') ?? 'Sin hora' }})</small>
                                            </span>
                                        </div>
                                        <div class="fs-6 py-2">
                                            <strong>{{ __('Observacion:') }}</strong>
                                            <span class="ms-2">{{ $info->observacion_responsable
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="fs-6 py-2">
                                            ...
                                            @if($info->tipo_campo === 'archivo')
                                                ...
                                                <div class="d-flex align-items-center bg-light-info rounded p-5 mb-3">
                                                    <span class="svg-icon svg-icon-2hx svg-icon-info me-5">
                                                        <i class="fas fa-file-{{ $info->info_solicitada->categoria == 'imagen' ? 'image' : ($info->info_solicitada->categoria == 'documento' ? 'alt' : 'video') }} fs-1 text-info"></i>
                                                    </span>
                                                    <div class="flex-grow-1">
                                                        <a href="#" class="fw-bold text-gray-800 text-hover-primary fs-6">{{$info->info_solicitada->nombre }}</a>
                                                        <span class="text-muted fw-semibold d-block">Tipo: {{ strtoupper($info->info_solicitada->categoria)}}</span>
                                                    </div>
                                                </div>
                                            @else
                                            <span class="ms-2">{{ $info->info_solicitada ?? 'No Especificado' }}</span>
                                            @endif
                                        </div>
                                        <div class="fs-6 py-2">
                                            <strong>{{ __('Fecha de Respuesta:') }}</strong>
                                            <span class="ms-2">{{ $info->fecha_respuesta_info?->format('d/m/Y') ?? 'N/A' }}
                                                <small class="text-muted">({{ $info->fecha_respuesta_info?->format('H:i') ?? 'Sin hora' }})</small>
                                            </span>
                                        </div>
                                        @endif
                ...
                if (idArea) {
                    ...
                    $.ajax({
                        ...
                        success: function(users) {
                            userSelect.empty();
                            userSelect.append(
                                '<option value="">No asignar a usuario específico (Solo al Área)...</option>'
                            );
                            ...
                            if (users.length > 0) {
                                $.each(users, function(index, user) {
                                    ...
                                    const isSelected = (user.id ==
                                        '{{ $denuncia->id_responsable }}');
                                    userSelect.append(new Option(user.name + ' (' + user.email +
                                        ')', user.id, isSelected, isSelected));
                                });
                            } else {
                                userSelect.append(
                                    '<option value="" disabled>No hay usuarios en esta área.</option>');
                            }

                            userSelect.prop('disabled', false);
                            userSelect.val(userSelect.find(':selected').val() || '').trigger(
                                'change'); // Mantener o limpiar
