...
<head>
    ...
    <style>
        body { font-family: sans-serif; font-size: 10pt; margin: 40px; }
        .header { text-align: center; margin-bottom: 20px; border-bottom: 2px solid #ccc; padding-bottom: 10px; }
        .header h1 { font-size: 16pt; color: #1e88e5; }
        .section-title { font-size: 12pt; margin-top: 15px; margin-bottom: 8px; padding: 5px; background-color: #f0f0f0; border-left: 5px solid #1e88e5; }
        .data-row { margin-bottom: 5px; }
        .data-row strong { display: inline-block; width: 120px; font-weight: bold; }
        .details-box { border: 1px solid #ddd; padding: 10px; margin-bottom: 15px; }
        .badge { background-color: #ff9800; color: white; padding: 3px 6px; border-radius: 3px; font-size: 8pt; }
    </style>
</head>
<body>
    ...
    <div class="header">
        ...
        <p><strong>Folio Único:</strong> {{ $denuncia->folio_seguimiento }} | 
           ...
           <strong>Estado:</strong> {{ $denuncia->estado->nombre ?? 'N/D' }}</p>
    </div>
    ...
    <div class="details-box">
        <p class="data-row"><strong>Fecha de Hechos:</strong> {{ $denuncia->circunstancia->fecha_hechos->format('d/m/Y') ?? 'N/A' }} ({{ $denuncia->circunstancia->hora_hechos ?? 'Sin hora' }})</p>
        <p class="data-row"><strong>Ubicación:</strong> {{ $denuncia->circunstancia->municipio->nombre_municipio ?? 'N/A' }}, {{ $denuncia->circunstancia->localidad ?? 'N/A' }}</p>
        <p class="data-row"><strong>Dirección Exacta:</strong> {{ $denuncia->circunstancia->direccion_exacta ?? 'N/A' }}</p>
        <p class="data-row"><strong>Dependencia Señalada:</strong> {{ $denuncia->circunstancia->dependencia_involucrada ?? 'N/A' }}</p>
        <p class="data-row"><strong>Trámite Relacionado:</strong> {{ $denuncia->circunstancia->tramite_solicitado ?? 'N/A' }}</p>
        <p style="margin-top: 10px;"><strong>Motivo de la Denuncia:</strong> {{ $denuncia->motivo_denuncia }}</p>
        <p style="margin-top: 10px;"><strong>Circunstancias Detalladas:</strong> {{ $denuncia->circunstancia->circunstancias_detalladas ?? 'Sin detalle adicional' }}</p>
    </div>
    ...
    @forelse ($denuncia->involucrados as $involucrado)
        <div class="details-box">
            <p><strong>Nombre:</strong> {{ $involucrado->nombre_denunciado ?? 'Desconocido' }} 
               @if($involucrado->es_servidor_publico)<span class="badge">Servidor Público</span>@endif
            </p>
            <p class="data-row"><strong>Puesto:</strong> {{ $involucrado->puesto_denunciado ?? 'N/A' }}</p>
            <p class="data-row"><strong>Descripción Física:</strong> {{ $involucrado->descripcion_fisica ?? 'Sin descripción.' }}</p>
        </div>
    @empty
    ...
    @if ($denuncia->es_anonima)
        <div class="details-box" style="background-color: #fdd;">
            <p style="text-align: center; font-weight: bold;">DENUNCIA ANÓNIMA. No hay datos de contacto registrados.</p>
        </div>
