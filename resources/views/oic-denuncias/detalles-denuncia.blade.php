                                    ...
                                    <div class="card-body">
                                        <div class="fs-6 py-2">
                                            ...
                                            <span class="ms-2">{{
                                                ...
                                                <small class="text-muted">({{ $denuncia->circunstancia->hora_hechos ??
                                                    'Sin hora' }})</small>
                                            </span>
                                        </div>
                                        <div class="fs-6 py-2">
                                            <strong>{{ __('Lugar (Municipio):') }}</strong>
                                            <span class="ms-2">{{ $denuncia->circunstancia->municipio->nombre_municipio
                                        ...
                                        <div class="fs-6 py-2">
                                            ...
                                            <span class="ms-2">{{ $denuncia->circunstancia->direccion_exacta ?? 'N/A'
                                                }}</span>
                                        </div>
                                        <div class="fs-6 py-2">
                                            <strong>{{ __('Dependencia Señalada:') }}</strong>
                                            <span class="ms-2">{{ $denuncia->circunstancia->dependencia_involucrada ??
                                                'N/A' }}</span>
                                        </div>
                                        <div class="fs-6 py-2">
                                            <strong>{{ __('Trámite / Solicitud:') }}</strong>
                                            <span class="ms-2">{{ $denuncia->circunstancia->tramite_solicitado ?? 'N/A'
                                                }}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="card card-dashed shadow-sm">
                                    ...
                                    <div class="card-body">
                                        <p><strong>{{ __('Circunstancias Detalladas:') }}</strong> {{
                                            $denuncia->circunstancia->circunstancias_detalladas ?? 'Sin detalle
                                            adicional' }}</p>
                                        <p><strong>{{ __('Programa Público:') }}</strong> {{ $denuncia->programa_publico
                                            ?? 'No relacionado a programa' }}</p>
                                        <p><strong>{{ __('Daño Económico:') }}</strong> <span
                                                class="badge badge-light-danger fs-5">${{
                                                number_format($denuncia->dinero_solicitado, 2) }}</span></p>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="tab-pane fade" id="tab_involucrados" role="tabpanel">
                        ...
                        <div class="card card-dashed mb-5 bg-light-primary">
                            ...
                            <div class="card-body py-3">
                                <div class="row fs-6">
                                    <div class="col-md-4"><strong>Puesto:</strong> {{ $involucrado->puesto_denunciado ??
                                        'N/A' }}</div>
                                    <div class="col-md-4"><strong>Sexo:</strong> {{ $involucrado->sexo ?? 'N/A' }}</div>
                                    <div class="col-md-4"><strong>Edad Aprox:</strong> {{ $involucrado->edad_aprox ??
                                        'N/A' }}</div>
                                    <div class="col-md-12 mt-3"><strong>Descripción Física:</strong> {{
                                        $involucrado->descripcion_fisica ?? 'Sin descripción adicional.' }}</div>
                                </div>
                            </div>
                        </div>
                        @empty
                        <div class="alert alert-info">{{ __('No se proporcionó información detallada sobre personas
                            involucradas o denunciadas.') }}</div>
                        @endforelse
                        ...
                        <div class="card card-dashed mb-5 bg-light-success">
                            <div class="card-header">
                                <h5 class="card-title">{{ $testigo->nombre_testigo ?? 'Testigo Anónimo/Sin Nombre' }}
                                </h5>
                            </div>
                            <div class="card-body py-3">
                                <p><strong>Contacto:</strong> {{ $testigo->datos_contacto ?? 'N/A' }}</p>
                                <p><strong>Observaciones:</strong> {{ $testigo->observaciones ?? 'Sin observaciones.' }}
                                </p>
                            </div>
                        </div>
                        @empty
                        <div class="alert alert-light">{{ __('El denunciante no proporcionó información sobre
                            testigos.') }}</div>
                        @endforelse
                    ...
                    <div class="tab-pane fade" id="tab_evidencia" role="tabpanel">
                        ...
                        <div class="d-flex align-items-center bg-light-info rounded p-5 mb-3">
                            ...
                            <div class="flex-grow-1">
                                ...
                                <span class="text-muted fw-semibold d-block">Tipo: {{ strtoupper($archivo->tipo_archivo)
                                    }} | Carga: {{ $archivo->fecha_carga->format('d/m/Y') }}</span>
                            </div>
                            <span class="ms-2">
                        ...
                        @empty
                        <div class="alert alert-warning">{{ __('No se adjuntaron archivos de evidencia en esta
                            denuncia.') }}</div>
                        @endforelse
                    </div>
                    ...
                    <div class="tab-pane fade" id="tab_contacto_info" role="tabpanel">
                        <div class="card card-dashed shadow-sm">
                            ...
                            <div class="card-body">
                                ...
                                @else
                                <div class="alert alert-warning">{{ __('Denuncia identificada, pero los datos de
                                    contacto no fueron guardados o son nulos.') }}</div>
                                @endif
                            </div>
                        </div>
                    </div>
