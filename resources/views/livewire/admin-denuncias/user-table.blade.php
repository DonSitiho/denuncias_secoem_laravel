        ...
        <table class="table align-middle table-row-dashed fs-6 gy-5">
            <thead>
                <tr class="text-start text-muted fw-bold fs-7 text-uppercase gs-0">
                    ...
                    <th class="min-w-150px cursor-pointer" wire:click="sortBy('email')">{{ __('Usuario (Correo)') }}
                    </th>
            ...
            <tbody class="text-gray-600 fw-semibold">
                ...
                <tr wire:key="{{ $user->id }}">
                    ...
                    <td>
                        ...
                        <div class="form-check form-switch form-check-custom form-check-solid">
                            <input class="form-check-input" type="checkbox" id="status_{{ $user->id }}" value="1" {{
                                $user->is_active ? 'checked' : '' }}
                            wire:click="toggleActive({{ $user->id }})"
                            wire:confirm="¿Estás seguro de que deseas {{ $user->is_active ? 'desactivar' : 'activar' }}
                            a este usuario?"
                            />
                    ...
                    <td>
                        ...
                        <a href="{{ route('admin.usuarios.edit', $user) }}"
                            class="btn btn-icon btn-light-warning btn-sm me-1" title="{{ __('Editar Usuario') }}">
                            <i class="fas fa-edit"></i>
                        </a>

                        {{-- Opcional: Botón para restablecer contraseña --}}
                        {{-- <button wire:click="resetPassword({{ $user->id }})"
                            class="btn btn-icon btn-light-info btn-sm" title="{{ __('Restablecer Contraseña') }}">
                ...
                @empty
                <tr>
                    <td colspan="6" class="text-center text-muted">{{ __('No se encontraron usuarios activos con área
                        asignada.') }}</td>
                </tr>
                @endforelse
            </tbody>
